    ...
    for (insn in instructions) {
        when (val opcode = insn.opcode) {
            ...
            Opcodes.INVOKEVIRTUAL, Opcodes.INVOKESPECIAL, Opcodes.INVOKEDYNAMIC -> {
                ...
                augmented.add(AssertNonNullTerminator(newPlaceholderBlock(), argsCnt, Check.INVK_NULL_REC))
            }

            Opcodes.GETFIELD, Opcodes.PUTFIELD -> {
                val stackDepth = if (opcode == Opcodes.GETFIELD) 0 else 1
                augmented.add(AssertNonNullTerminator(newPlaceholderBlock(), stackDepth, Check.FLD_NULL_OWNER))
            }

            in Opcodes.IALOAD..Opcodes.SALOAD -> {
                augmented.add(AssertNonNullTerminator(newPlaceholderBlock(), 1, Check.INDEXING_NULL_ARRAY))
                augmented.add(AssertInBoundsTerminator(newPlaceholderBlock(), isStore = false))
            }

            in Opcodes.IASTORE..Opcodes.SASTORE -> {
                augmented.add(AssertNonNullTerminator(newPlaceholderBlock(), 2, Check.INDEXING_NULL_ARRAY))
                augmented.add(AssertInBoundsTerminator(newPlaceholderBlock(), isStore = true))
            }

            Opcodes.NEWARRAY, Opcodes.ANEWARRAY, Opcodes.MULTIANEWARRAY -> {
                val nDims = if (insn is MultiANewArrayInsnNode) insn.dims else 1
                augmented.add(AssertValidArrayLengthTerminator(newPlaceholderBlock(), nDims))
            }

            Opcodes.IDIV, Opcodes.LDIV, Opcodes.IREM, Opcodes.LREM -> {
                augmented.add(IsValidDivisorTerminator(newPlaceholderBlock()))
            }
